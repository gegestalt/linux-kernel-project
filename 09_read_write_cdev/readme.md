...
[`gdb_walkthrough.md`](gdb_walkthrough.md). Covers the four-step modern
registration sequence, the write-side short-write/`-ENOSPC` distinction,
the read-side EOF-at-`data_len` boundary, and `rw_llseek`'s pass-through
into the generic `fixed_size_llseek()` helper.
