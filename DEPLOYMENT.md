...
Every lab in this repo ends at `sudo insmod ./module.ko` — you built it,
you loaded it, on your own machine, with your own kernel. That's the
...
signing (why every lab's `dmesg` output has already shown you a taint
warning), and the actual upstream contribution pipeline this repo's
`checkpatch.pl` habit has been rehearsing pieces of since lab 05.

## The taint warning you've already seen

Load any lab module and `dmesg` shows something like:
...
| Option | Effect |
|---|---|
| `CONFIG_MODULE_SIG_FORCE` | Off (default): unsigned/unverifiable modules load anyway but taint the kernel — exactly what every lab in this repo has been doing. On: they're rejected outright. |
...
lab like this repo** — `CONFIG_MODULE_SIG_FORCE=off` (the common default)
is specifically permissive for exactly this use case. It becomes real
the moment you're building for a machine (or a distro) with
...
Every lab here is built against `/lib/modules/$(uname -r)/build` — one
...
None of this repo's labs set `MODULE_VERSION()` — worth adding once a
...
Labs 05, 07, 08, and every lab from 09 onward wire
