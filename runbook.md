...
Run all of this inside a disposable VM, not bare metal: modules 11 onward
(locking, blocking I/O, kernel threads) can hang or panic the machine
instead of just crashing a process, and module 14's entry below in
particular is only safe there.
...
which gpiodetect gpiofind             # from libgpiod tools, needed for module 03
lsmod | grep gpio_sim || sudo modprobe gpio-sim   # confirm the gpio-sim module loads
grep CONFIG_DEBUG_FS /boot/config-$(uname -r)      # needed for module 16's debugfs half
grep CONFIG_DEBUG_ATOMIC_SLEEP /boot/config-$(uname -r)  # informational, for module 14
```
...
A generic per-module cycle, if you'd rather skip ahead to a specific one:
...
```
**Look for:** the same greeting as module 01, but now `modinfo` actually
shows `author:` and `description:` fields — module 01's didn't.
```bash
...
`/proc/sys/kernel/printk`; see this module's README for that experiment.
...
"deliberately unsafe" exercise in this module's own README needs the VM.)*
```bash
...
contrast is the whole point of this module.
```bash
...
`9999`). That silent-vs-logged, validated-vs-raw contrast is the whole
...
The per-module `readme.md` is the source of truth over this file for
...
each module down to "build, run, watch for X." If a `make` or `insmod`
step fails, check that module's README first; if the *expected output*
doesn't match what's described here, that's worth treating as a real bug report
