...
- Contrast with module 03/[16](../16_debugfs_sysfs/): a manually-written
...
| `Makefile` | Build, `clean`, `check`/`checkpatch` (same shape as modules 05/07/08). |
...
- Set `repeat_count` to something absurd, like `999999`, via sysfs, then
  ...
  same principle module 03's `poll_ms` range check demonstrates: a `0644`
...
[`gdb_walkthrough.md`](gdb_walkthrough.md). This is the module where you
watch parameter values sitting correctly in memory *before* the init
function has run a single line of its own body — proof `insmod` parses
and stores them ahead of calling into your code at all.
