...
that's worth seeing deliberately, once, before every later lab takes
...
to `init_module`, in which case this lab would debug identically to 01
and there'd be nothing new to see. Check that assumption against the
...
function — is identical between labs 01 and 02; don't expect a
...
lab, because only this lab's macro-generated alias leaves the original
semantic name in the symbol table for GDB to find.
...
every lab in this repo, is unloaded as a whole unit instead, so the
annotation has no visible effect on any lab here — but every real driver
...
Same two sessions as every lab — `vmb` (guest) and `gdbsess` (GDB) —
...
This is the same shape you'd see in lab 01 — `do_one_initcall()` calls
...
Exit path, same shape as lab 01:
...
runtime behavior really is the same as lab 01's. The one genuine
...
the one generic name `init_module` the way lab 01's did. Every lab from
here on uses this modern pattern and its real function names; this is
the lab that establishes why that's safe to rely on.
