...
bug, it's to build the *mechanical* muscle memory every later lab
...
later lab's walkthrough assumes you've done this one first.
...
exactly why lab 01 is the right place to learn the "module isn't loaded
yet" problem, because `init_module` is also the literal name of the
generic kernel function every module's init funnels through
(`kernel/module/main.c`'s `do_init_module()` calls a function pointer
it names `init_module` internally) — so for this one lab, and only this
one, you'll see two different `init_module`s at different points in the
same session.
...
Build this lab against the debug tree, not your host kernel:
...
Two things that generalize to every other lab in this repo: first, an
...
this lab builds on exactly this mechanism, aimed at less trivial code.
