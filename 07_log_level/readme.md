...
[`gdb_walkthrough.md`](gdb_walkthrough.md). This module's own premise —
"`dmesg -w` isn't the whole picture" — is exactly what `lx-dmesg` fixes:
it reads the ring buffer straight out of kernel memory, so it works even
with the guest frozen mid-breakpoint.
