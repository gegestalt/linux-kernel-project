...
`producer_thread_fn` only "enters" once for the thread's entire
lifetime (it loops internally forever), so a kprobe on it only fires
once, right at thread creation — not useful for watching the recurring
loop. The organic fix: **filter by PID** instead of by function, and
probe the core scheduler function the loop actually calls into every
iteration:

```bash
PID=$(cat /sys/kernel/kthreads_demo/status | grep -oP 'pid=\K[0-9]+')
echo "producer pid: $PID"
sudo bpftrace -e '
kprobe:schedule /pid == '"$PID"'/ { printf("kthread[%d] entering schedule() - genuinely sleeping\n", pid); }
' &
sleep 4
```

Real captured output over ~4 seconds (`interval_ms=300`):

```
producer pid: 179525
kthread[179525] entering schedule() - genuinely sleeping
kthread[179525] entering schedule() - genuinely sleeping
kthread[179525] entering schedule() - genuinely sleeping
kthread[179525] entering schedule() - genuinely sleeping
kthread[179525] entering schedule() - genuinely sleeping
...
Fifteen hits over 4 seconds at a 300ms interval — matches exactly.
bpftrace's `/pid == N/` filter predicate traces *everything a specific
task does*, regardless of which function it's nominally "inside" —
strictly more useful here than trying to name a specific function ahead
of time, and simpler than the raw `set_ftrace_pid` mechanism underneath
it (see `../FTRACE_TRACING.md`).
