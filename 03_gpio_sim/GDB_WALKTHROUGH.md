...
`gpioctrl.c` is the first lab with real runtime complexity: it owns a
...
labs 01/02, most of this driver's interesting code does **not** run
during `insmod` at all — `gpioctrl_work_fn()` runs later, from a
`kworker` thread, asynchronously, driven by the workqueue rather than
by anything you typed. That makes this lab the right place to learn a
technique the earlier labs didn't need: catching a breakpoint that
...
for this lab (see [`../GDB_DEBUGGING.md`](../GDB_DEBUGGING.md)'s config
...
earlier lab to get `lx-symbols` to see this module. This walkthrough
picks up from there.
...
You should see this lab's own `pr_info(DRIVER_NAME ": init: completed
in ... us\n")` line at the very end of the ring buffer.

### Step 2 — catch the workqueue callback firing on its own

This is the technique lab 01/02 never needed. `gpioctrl_work_fn()`
...
**Read this backtrace carefully — it's the whole point of the lab.**
...
a whole lab to, seen here in a real driver rather than an artificial
stress test:
...
during a lab session isn't worth reasoning about further.
...
someone else's schedule (a workqueue, in this lab; a timer in 14, a
