...
session creation, gpio-sim setup inside the guest, build, boot, every
gdb command, every expected output, and cleanup, start to finish, no
other file needed: [`gdb_walkthrough.md`](gdb_walkthrough.md). This is
the module where stepping through GDB pays off most — you watch a button
read turn into an LED write, one line at a time, including a callback
that fires on its own from a kernel worker thread, not from anything you
typed.
